...
def plot_landing_path(points, a, x0, y0, score):
    ...
    # x_curve = np.linspace(min(x_vals) - 0.3, max(x_vals) + 0.3, 300)
    # y_curve = x_curve**2 + 0.04
    # plt.plot(x_curve, y_curve, 'r--', label='Parabolic boundary (y = $x^2 + 0.04$)')
    ...
    plt.text(x0, 1.6, f'Score: {score:.2f}%', fontsize=12, verticalalignment='bottom', horizontalalignment='center')
    plt.show()
